...
func init() {
	...
	payloadCmd.Flags().StringVarP(&PayloadHost, "host", "h", "", "Host IP used in payload")
	payloadCmd.Flags().StringVarP(&PayloadPort, "port", "p", "4444", "Port used in the payload")
	payloadCmd.Flags().StringVarP(&PayloadHost, "language", "l", "php", "The language the payload must be generated in")

	payloadCmd.Flags().BoolVarP(&PayloadForceFile, "forcefile", "ff", false, "Force the output to be formatted in a file.")

	payloadCmd.MarkFlagRequired("host")
	payloadCmd.MarkFlagRequired("language")
}

type shell struct {
	Payload   string
	IsFile    bool // Always put the output in a file
	Extension string
}
